...
class AQIServer(Transformer):
    ...
    def update(self, updatedata=None) -> dict:
        ...
        if updatedata is not None:
            ...
            for pollutant in self.AQIData['pollutants'].keys():
                raw_value = updatedata["list"][0]["components"][pollutant]
                logging.debug(f"Raw value for {pollutant}: {raw_value}")
                converted_value = self._convert_reading(raw_value, pollutant)
                logging.debug(f"Converted value for {pollutant}: {converted_value}")
                scaled_value, row = self._scaled_reading(converted_value, pollutant)
            ...
            values = {
                ...
                'aqi_score': max_score,
                'aqi_adjective': self.AQIData["aqi"]["adjectives"][max_row],
                'color': self.AQIData["aqi"]["colors"][max_row],
                'main_pollutant': self.AQIData["pollutants"][max_pollutant]["name"]
            }
    ...
    def _convert_reading(self, val: float|int, pol: str) -> float|int:
        """
        ...
        """
        units    = self.AQIData['pollutants'][pol]["units"]
        decimals = self.AQIData['pollutants'][pol]["decimals"]
        weight   = self.AQIData['pollutants'][pol]["weight"]
    ...
    def _scaled_reading(self, cval: float|int, pol: str) -> int:
        """
        ...
        """
        aq = self.AQIData['aqi']['values']
        pm = self.AQIData['pollutants'][pol]['values']
        scaled = 0
        row = 0
        for i in range(6):
